...
if ! command_exists node; then
  log_message "ERROR: Node.js is not installed. Please install Node.js (v16+) from https://nodejs.org"
  exit 1
fi
log_message "✓ Node.js found: $(node --version)"
...
if command_exists prettier; then
  ...
  log_message "Installing Prettier..."
...
log_message "Checking for TypeScript Language Server..."
if command_exists typescript-language-server; then
  TLS_VERSION=$(typescript-language-server --version)
  log_message "✓ TypeScript Language Server is already installed: $TLS_VERSION"
else
  log_message "Installing TypeScript Language Server..."
  npm install --global typescript-language-server@latest
  log_message "✓ TypeScript Language Server installed: $(typescript-language-server --version)"
fi
...
if command_exists eslint; then
  ...
  log_message "Installing ESLint and related packages..."
