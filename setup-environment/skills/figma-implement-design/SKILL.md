...
### Step 0: Verify Figma MCP Connection

Before starting, verify that the Figma MCP server is connected and accessible.

**In Cursor:** The Figma MCP server should be configured as `user-Figma` in your MCP settings. You can verify this by checking if Figma tools are available via the `CallMcpTool` function with `server: "user-Figma"`.

**If not configured:**

1. Open Cursor Settings → MCP Servers
2. Add a new MCP server with:
   - Name: `Figma` (or `user-Figma`)
   - URL: `https://mcp.figma.com/mcp`
3. Authenticate when prompted
4. Restart Cursor to complete setup

**Available tools via `user-Figma` MCP:**
- `get_design_context` - Get structured design data
- `get_screenshot` - Capture visual reference
- `get_metadata` - Get high-level node structure
- `get_variable_defs` - Get design token definitions
- `whoami` - Verify authentication status
